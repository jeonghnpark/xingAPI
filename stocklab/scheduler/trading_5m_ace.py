...
def check_sell_order(code):
    ...
    for order in sell_order_list:
        ...
        check_result = ebest.order_check(order_no)  # 매도주문쿼리
...
def trading_scenario(code_list):
    for code in code_list:
        ...
        print(f"current price of {result[0]['hname']} is {current_price}, and offer price is {offerho1}")
        ...
        if buy_order_cnt == 0:  #'status':buy_ordered 가 없을시 (as a strategy) 신규매수
            order = ebest.order_stock(code, "2", offerho1, "2", "00")
...
# ebest = EBest("DEMO")  # should be global since each process calls it
# ebest.login()
# mongo = MongoDBHandler()
